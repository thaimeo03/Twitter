...
const MYID = new ObjectId('64905aaf60bef1d11468bda5')
...
const unfollowMultipleUsers = async () => {
  await databaseService.followers.deleteMany({ user_id: new ObjectId('64ae518e12de778b00d04657') })
}
...
const insertTweet = async (user_id: string, body: TweetBody) => {
...
const insertMultipleTweets = async (ids: ObjectId[]) => {
  ...
  const result = await Promise.all(
    ids.map(async (id, index) => {
      await Promise.all([
        insertTweet(id.toString(), createRandomTweet()),
        insertTweet(id.toString(), createRandomTweet())
      ])
      count += 2
