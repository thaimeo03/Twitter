...
export const getTweetController = async (req: Request, res: Response) => {
  ...
  const { guest_views, user_views, updated_at } = await tweetsService.increaseView(
    req.params.tweet_id,
...
export const getTweetChildrenController = async (req: Request, res: Response) => {
  ...
  const { result, total_pages } = await tweetsService.getTweetChildren({ tweet_id, tweet_type, page, limit, user_id })
  return res.json({
...
export const getNewFeedsController = async (req: Request, res: Response) => {
  ...
  const page = Number(req.query.page) || 1
  const limit = Number(req.query.limit) || 5
  const { result, total_pages } = await tweetsService.getNewFeeds({ user_id, page, limit })
